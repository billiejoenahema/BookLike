...
<ul class="list-group">
  ...
  <li class="list-group-item p-0">
    ...
    <div class="card-header p-3 border-bottom-0 bg-white d-flex flex-column">
      <div class="d-flex flex-column text-right">
        <span class="text-secondary">{{ $comment->created_at->format('Y/m/d')
          }}</span>
      </div>
  ...
  @endforelse
  <!-- Comment Post Modal -->
  <div class="modal fade form-group row m-auto" id="postCommentModal"
    tabindex="-1" role="dialog" aria-labelledby="postCommentModalTitle"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header border-bottom-0">
          <img src="{{ $storage->url($login_user->profile_image) }}"
            class="rounded-circle" width="48" height="48" loading='lazy' />
        ...
        <form method="POST" action="{{ route('comments.store') }}"
          ...
          <div class="modal-body py-0">
            ...
            <textarea class="
              ...
              is-invalid
              @enderror
            " name="text" id="textarea" required autocomplete="text" rows="6"
              autofocus onkeyup="checkTextLength()">{{ old("text") }}</textarea>
            @error('text')
          ...
          <div class="modal-footer border-top-0">
            ...
            <button type="submit" id="postButton" class="
              btn btn-primary
              rounded-pill
              ml-4
              shadow-sm
...
<div class="text-right mb-5">
  <button type="button"
    ...
    data-toggle="modal" data-target="#postCommentModal">
    コメントを投稿する
  </button>
</div>
