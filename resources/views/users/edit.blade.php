...
<div class="card shadow-sm">
  <div class="card-header d-flex justify-content-between">
    <div>プロフィールを編集</div>
    @if($login_user->id !== 1)
    <a href="#" class="text-decoration-none text-danger" data-toggle="modal"
      data-target="#deleteComfirmModal" role="button" title="アカウント削除">
  ...
  <div class="card-body px-sm-5 px-md-3">
    ...
    @endif
    <form method="POST" action="{{ url('users/' .$login_user->id) }}"
      ...
      <div class="d-flex flex-wrap form-group align-items-center">
        ...
        <div class="col-md-7 d-flex align-items-center">
          <img src="{{ $storage->url($login_user->profile_image) }}"
            class="rounded-circle mr-3" width="48" height="48" loading='lazy' />
      ...
      <div class="form-group row">
        ...
        <div class="col-md-7">
          <input id="screen_name" type="text" class="
            form-control
            @error('screen_name')
            is-invalid
            @enderror
          " name="screen_name" value="{{ $login_user->screen_name }}" required
      ...
      <div class="form-group row">
        ...
        <div class="col-md-7">
          <input id="name" type="text"
            ...
            value="{{ $login_user->name ?? '' }}" autocomplete="name"
            autofocus />
          @error('name')
          <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
      ...
      <div class="form-group row">
        ...
        <div class="col-md-7">
          <input id="email" type="email" class="
            ...
            is-invalid
            @enderror
          " name="email" value="{{ $login_user->email }}" required
            autocomplete="email" />
          @error('email')
          <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
      ...
      <div class="form-group row">
        ...
        ">{{ __("好きなジャンル") }}</label>
        <div class="col-md-7">
          <textarea id="category" type="text" class="form-control"
            name="category" rows="2">{{ $login_user->category }}</textarea>
        </div>
      </div>
      {{-- 自己紹介文 --}}
      <div class="form-group row">
      ...
      <div class="form-group row py-3 mb-0">
        <div class="col-md-7 offset-md-4 d-flex justify-content-between">
          ...
          @if($login_user->id == 1)
          <button type="button"
            class="btn btn-primary rounded-pill shadow-sm disabled" disabled>
            更新する
          </button>
          @else
