...
function Timeline(props) {
    ...
    const { timelines, loginUser, storage, root } = props
            ...
            { timelines.map((timeline) => (
                <div className="card shadow-sm mb-3" key={timeline.id}>
                    <div className="card-haeder p-3 d-flex">
                        <a href={`${root}/users/${timeline.user.id}`} className="text-reset">
                    ...
                    <div className="card-body border-top border-bottom py-0">
                        <a href={`${root}/reviews/${timeline.id}`} className="d-block text-reset text-decoration-none">
                    ...
                    <div className="card-footer py-1 d-flex justify-content-end bg-white">
                        ...
                        <div className="mr-3 d-flex align-items-center">
                            {(() => {
                                if (timeline.user.id === loginUser.id) {
                                    return (
                                        <a href={`${root}/reviews/${timeline.id}/edit`}
                        ...
                        {/* コメントボタン */}
                        <div className="mr-3 d-flex align-items-center">
                            <a href={`${root}/reviews/${timeline.id}`} data-tip="コメントを投稿"><i className="far fa-comment fa-fw text-blog"></i>
                                <ReactTooltip effect="float" type="info" place="top" /></a>
                            <p className="mb-0 text-secondary">{timeline.comments.length}</p>
                        </div>
                        {/* いいねボタン */}
                        <div className="mr-3 d-flex align-items-center">
                            <FavoriteButton timeline={timeline} loginUser={loginUser} root={root} />
                        </div>
                    </div>
                </div>
            ))}
